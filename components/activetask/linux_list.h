 ...
 * @Date        : 2022-10-17 10:31:53
 * @LastEditors : kevin.z.y <kevin.cn.zhengyang@gmail.com>
 * @LastEditTime: 2022-10-17 10:31:53
 * @FilePath    : /activetask/components/linux_list.h
			      ...
			      struct list_head *next)
{
	...
	new_list->next = next;
	new_list->prev = prev;
	prev->next = new_list;  // WRITE_ONCE(prev->next, new);
}

/**
 * list_add - add a new entry
 * @new_list: new entry to be added
 ...
 * @old_list : the element to be replaced
 * @new_list : the new element to insert
 *
 * If @old was empty, it will be overwritten.
 */
static inline void list_replace(struct list_head *old_list,
 ...
 * If @old was empty, it will be overwritten.
 */
static inline void list_replace_init(struct list_head *old_list,
				     struct list_head *new_list)
{
...
{
	return head->next == head;      //READ_ONCE(head->next) == head;
}
...
{
	__list_del_entry(entry);
	entry->prev = entry;    //WRITE_ONCE(entry->prev, entry);
